...
void pwm_init (pwm_channel_enum pin, const uint32 freq, const uint32 duty)
{
    ...
    uint16 freq_div = ((bus_clock / freq) >> 15);                               // 计算预分频  120HZ-30
//    printf("\r\nbus_clock=%d",bus_clock);
//		printf("\r\nfreq_div=%d",freq_div);
		uint16 period_temp = (bus_clock / freq / (freq_div + 1));                   // 计算自动重装载值 120HZ-32258
//		printf("\r\nperiod_temp=%d",period_temp);
